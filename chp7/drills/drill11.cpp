...
const string quitkey = "exit";                              //declaration of exit like we did for let               --drill11
...
Token Token_stream::get()
{
	...
	{
		case print:
		//case quit:                                       no need quit as we will check exit string
		case '(':
		case ')':
		case '+':
		case '-':
    	...
    	default: 
    	{
    		if (isalpha(ch))
    		{
                ...
                else if (s == quitkey) return Token{quit};                                            //quitkey checking --drill11
