...
{
    char ch;                                            //since we need to check open paranthesis 
    if (cin.get(ch) && ch != '(') 
        error("'(' is required at starting of sqrt");
    cin.putback(ch);                                    //need to putback the last entry so that we may check sqrt is ending with ')'
...
double primary()
{
	...
	switch (t.kind)
	{
		...
		case '+':
			return primary();
        case sqrttoken:
            return sqrt_calculation();                              //inserting in primary so that grammer rules are followed as it is primary function
		default:
